...
for i = 1:length(datasets)
    allPBIAS = [allPBIAS; datasets{i}.PBIAS];
end
%ymin = min(allR2) * 0.98;
%ymax = max(allR2) * 1.02;
...
for i = 1:length(datasets)
    ...
    % --- Plot line first ---
    plot(T.Num_Features, T.PBIAS, 'Color', lineColor, 'LineWidth',2);
    hold on
    
    % Scatter points colored same as line
    scatter(T.Num_Features, T.PBIAS, 40, lineColor, 'filled', 'MarkerFaceAlpha', 0.6);
...
text(xOffset, yOffset, panelLabels{i}, 'Units', 'normalized', ...
   ...
   % labelStr = sprintf('Min RMSE: %.4f\nIteration: %d', mp.RMSE(1), mp.Iteration(1));
    ...
    ylabel(t, 'PBIAS (%)', 'Interpreter', 'latex', 'FontName','Times New Roman','FontSize',20)
