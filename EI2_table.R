...
varimp_data <- data.frame(
  Code = c("cly_pc_sav", "gauge_lon"),
  Attribute_Name = c("Clay Content Percentage", "Gauge Longitude"),
  Category = c("Soils & Geology", "Metadata"),
...
varimp_table <- varimp_data %>%
  gt() %>%
  tab_header(
    title = md("*Variable Importance for the 2-Year Pre- vs. Post-Forest Loss Evaporative Index (EI) Analysis Window*")
  ) %>%
  cols_label(
