#NetCDF Camels Great Britain extract
#Forest loss study
...
setwd("D:/Forest_loss/Caravan/timeseries/netcdf/camelsgb")

gage_loss_yrs=read.csv("D:/Forest_loss/Caravan/camels_gb_loss1_yr.csv")
gage_loss_yrs[c("camels", "num")] = do.call(rbind, strsplit(gage_loss_yrs$gauge_id, "_"))

for (k in 1:407)
{
  nc_data = nc_open(paste("camelsgb_",gage_loss_yrs$num[k],".nc",sep=""))
  ...
  time_obs=as.Date(date, origin = '1981-01-02')
...
write.csv(gage_loss_yrs,"D:/Forest_loss/Catchment_stats/5yr/ForestLoss_CatchmentStats_GB_5yr.csv")
