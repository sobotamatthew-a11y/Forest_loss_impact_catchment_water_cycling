...
setwd("D:/Forest_loss/Caravan/timeseries/netcdf/lamah")

gage_loss_yrs=read.csv("D:/Forest_loss/Caravan/camels_lamah_loss_yr.csv")
gage_loss_yrs[c("camels", "num")] = do.call(rbind, strsplit(gage_loss_yrs$gauge_id, "_"))

for (k in 1:5)
{
  nc_data = nc_open(paste("lamah_",gage_loss_yrs$num[k],".nc",sep=""))
  ...
  Before1 = Before[Before$year > lossyear - 2, ]
  After1 = After[After$year >= lossyear + 1 & After$year < lossyear + 2, ]
  After2 = After[After$year >= lossyear + 2 & After$year < lossyear + 3, ]
...
write.csv(gage_loss_yrs,"D:/Forest_loss/Catchment_stats/5yr/ForestLoss_CatchmentStats_LAMAH_5yr.csv")
