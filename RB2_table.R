...
varimp_data <- data.frame(
  Code = c("gauge_lon", "gdp_us_ssu", "dis_m3_pyr", "gauge_lat", "low_prec_freq", 
           "rdd_mk_sav", "loss_area_sqkm", "area_sqkm"),
  Attribute_Name = c("Gauge Longitude", "Gross Domestic Product", 
                     "Annual Mean Discharge", "Gauge Latitude", 
                     "Low Precipitation Frequency", "Road Density", 
                     "Total Forest Loss Area", "Drainage Area"),
  Category = c("Metadata", "Socioeconomic", "Hydrology", "Metadata", "Climate", "Land Cover", 
               "Land Cover", "Physiography"),
  Description = c("The geographic longitudinal coordinate of the streamflow gauge.",
                  "Estimated GDP (USD) within the sub-basin area.",
                  ...
                  "Total length of roads per unit area within the catchment.",
                  "Total physical footprint of canopy loss area.",
                  "The total surface area of the catchment that drains to the gauge.")
)
...
varimp_table <- varimp_data %>%
  gt() %>%
  tab_header(
    title = md("*Variable Importance for the 2-Year Pre- vs. Post-Forest Loss RB-Index (RB) Analysis Window*")
  ) %>%
  cols_label(
