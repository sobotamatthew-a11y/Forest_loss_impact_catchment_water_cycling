...
%% Directory
dataDir = 'D:\Forest_loss\HPC\030926_result';
...
for i = 1:length(datasets)
    allMASE = [allMASE; datasets{i}.MASE];
end
%ymin = min(allR2) * 0.98;
%ymax = max(allR2) * 1.02;
...
xOffset = 0.85;   % 2% from left
yOffset = 0.2;   % slightly below top (less than 1)
...
for i = 1:length(datasets)
    ...
    % --- Plot line first ---
    plot(T.Num_Features, T.MASE, 'Color', lineColor, 'LineWidth',2);
    hold on
    
    % Scatter points colored same as line
    scatter(T.Num_Features, T.MASE, 40, lineColor, 'filled', 'MarkerFaceAlpha', 0.6);
    
    % Highlight minimum RMSE in red
    %scatter(mp.Num_Features, mp.R2, 75, colors(5,:), 'filled', 'MarkerEdgeColor','k');
...
text(xOffset, yOffset, panelLabels{i}, 'Units', 'normalized', ...
   ...
   % labelStr = sprintf('Min RMSE: %.4f\nIteration: %d', mp.RMSE(1), mp.Iteration(1));
    ...
    ylabel(t, 'Mean Absolute Scaled Error', 'Interpreter', 'latex', 'FontName','Times New Roman','FontSize',20)
